    ...
    <div className="absolute inset-0 z-0 bg-[#020617] overflow-hidden">
        ...
        <style>
            {` 
                ...
                .orb-purple {
                    background: radial-gradient(circle at 30% 30%, #a78bfa, #8b5cf6, #5b21b6);
                    box-shadow: 0 0 100px rgba(139, 92, 246, 0.4);
                }
                .orb-indigo {
                    background: radial-gradient(circle at 30% 30%, #818cf8, #6366f1, #3730a3);
                    box-shadow: 0 0 100px rgba(99, 102, 241, 0.4);
                }
                .orb-pink {
                    background: radial-gradient(circle at 30% 30%, #f472b6, #db2777, #9d174d);
                    box-shadow: 0 0 100px rgba(219, 39, 119, 0.4);
                }
                .orb-violet {
        ...
        {/* Floating Bright 3D Orbs - Restricted to Pink, Mauve, and Blue */}
        <div className="orb-3d orb-blue w-96 h-96 -top-[10%] -left-[5%]" style={{ animationDelay: '0s' }} />
        <div className="orb-3d orb-pink w-[500px] h-[500px] -bottom-[10%] -right-[10%]" style={{ animationDelay: '-5s' }} />
        <div className="orb-3d orb-pink w-80 h-80 top-[10%] right-[20%]" style={{ animationDelay: '-8s' }} />
        <div className="orb-3d orb-violet w-64 h-64 bottom-[30%] left-[40%]" style={{ animationDelay: '-12s' }} />
        <div className="orb-3d orb-blue w-80 h-80 top-[30%] left-[60%]" style={{ animationDelay: '-10s' }} />
        <div className="orb-3d orb-violet w-64 h-64 bottom-[20%] left-[20%]" style={{ animationDelay: '-15s' }} />
...
const DefaultLogo = () => ( <LyzardLogo size={380} variant="dark" className="drop-shadow-[0_0_25px_rgba(168,85,247,0.7)]" /> );
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "Lyzard AI" }: AuthComponentProps) => {
  const { language } = useLanguage();
  const t = translations[language].auth;

  const modalSteps = [
    { message: t.signingUp, icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: t.onboarding, icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: t.finalizing, icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: t.welcomeAboard, icon: <PartyPopper className="w-12 h-12 text-green-500" /> }
  ];
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
        setModalErrorMessage(t.passwordMatchError || (language === 'fr' ? "Les mots de passe ne correspondent pas !" : "Passwords do not match!"));
    ...
    <AnimatePresence>
        {modalStatus !== 'closed' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md">
                <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a]/80 backdrop-blur-2xl border border-white/10 rounded-3xl p-10 w-full max-w-sm flex flex-col items-center gap-6 mx-4 shadow-2xl">
                    <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 via-transparent to-purple-500/10 rounded-3xl pointer-events-none" />
                    {(modalStatus === 'error' || modalStatus === 'success') && <button onClick={closeModal} className="absolute top-4 right-4 p-2 text-white/40 hover:text-white transition-colors"><X className="w-5 h-5" /></button>}
                    
                    {modalStatus === 'error' && <>
                        <div className="p-4 rounded-full bg-red-500/10 border border-red-500/20"><AlertCircle className="w-10 h-10 text-red-500" /></div>
                        <p className="text-xl font-semibold text-white text-center">{modalErrorMessage}</p>
                        <GlassButton onClick={closeModal} size="sm" className="mt-2 w-full">{t.tryAgain}</GlassButton>
                    </>}
                    ...
                    {modalStatus === 'loading' && 
                        <div className="flex flex-col items-center gap-6">
                            <div className="relative w-20 h-20">
                                ...
                                <div className="absolute inset-2 rounded-full border-b-2 border-purple-400 animate-spin-slow" />
                            </div>
                            <TextLoop interval={TEXT_LOOP_INTERVAL} stopOnEnd={true}>
                                {modalSteps.slice(0, -1).map((step, i) => 
                                    <p key={i} className="text-xl font-medium text-white/90 text-center tracking-tight">{step.message}</p>
                                )}
                            </TextLoop>
                        </div>
                    }
                    ...
                    {modalStatus === 'success' &&
                        <div className="flex flex-col items-center gap-6">
                            <div className="p-4 rounded-full bg-green-500/10 border border-green-500/20 shadow-[0_0_40px_rgba(34,197,94,0.2)]"><PartyPopper className="w-12 h-12 text-green-500" /></div>
                            <div className="text-center space-y-2">
                                <p className="text-2xl font-bold text-white tracking-tight">{modalSteps[modalSteps.length - 1].message}</p>
                                <p className="text-white/50 text-sm">{t.accountCreated}</p>
                            </div>
                            <GlassButton onClick={closeModal} size="sm" className="w-full">{t.getStarted}</GlassButton>
                        </div>
                    }
                </motion.div>
            </motion.div>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-6xl sm:text-7xl md:text-8xl tracking-tight bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent whitespace-nowrap mb-4">{t.welcomeHome}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 2}><p className="text-sm font-medium text-white/60">{t.continueWith}</p></BlurFade>
                    ...
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent whitespace-nowrap">{t.createPassword}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-white/60">{t.passwordLength}</p></BlurFade>
                    </motion.div>}
                     {authStep === "confirmPassword" && <motion.div key="confirm-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                         <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent whitespace-nowrap">{t.oneLastStep}</p></div></BlurFade>
                         <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-white/60">{t.confirmPasswordToContinue}</p></BlurFade>
                    </motion.div>}
                </AnimatePresence>
                ...
                <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-6">
                        ...
                        {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-6">
                            <BlurFade delay={authStep === 'email' ? 0.25 * 5 : 0} inView={true} className="w-full">
                                <div className="relative w-full">
                                    <AnimatePresence>
                                        {authStep === "password" && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.4 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-white/60 font-semibold">{t.email}</label></motion.div>}
                                    </AnimatePresence>
                                    <div className="glass-input-wrap w-full"><div className="glass-input">
                                        <span className="glass-input-text-area"></span>
                                        <div className={cn( "relative z-10 flex-shrink-0 flex items-center justify-center overflow-hidden transition-all duration-300 ease-in-out", email.length > 20 && authStep === 'email' ? "w-0 px-0" : "w-10 pl-2" )}><Mail className="h-5 w-5 text-white/90 flex-shrink-0" /></div>
                                        <input type="email" placeholder={t.email} value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={handleKeyDown} className={cn("relative z-10 h-full w-0 flex-grow bg-transparent text-white placeholder:text-white/50 focus:outline-none transition-[padding-right] duration-300 ease-in-out delay-300", isEmailValid && authStep === 'email' ? "pr-2" : "pr-0")} />
                                        <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isEmailValid && authStep === 'email' ? "w-10 pr-1" : "w-0" )}><GlassButton type="button" onClick={handleProgressStep} size="icon" aria-label="Continue with email" contentClassName="text-white/90 hover:text-white"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                    </div></div>
                                </div>
                            </BlurFade>
                            <AnimatePresence>
                                {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                    <div className="relative w-full">
                                        <AnimatePresence>
                                            {password.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-white/60 font-semibold">{t.password}</label></motion.div>}
                                        </AnimatePresence>
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            <span className="glass-input-text-area"></span>
                                            <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                                {isPasswordValid ? <button type="button" aria-label="Toggle password visibility" onClick={() => setShowPassword(!showPassword)} className="text-white/90 hover:text-white transition-colors p-2 rounded-full">{showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button> : <Lock className="h-5 w-5 text-white/90 flex-shrink-0" />}
                                            </div>
                                            <input ref={passwordInputRef} type={showPassword ? "text" : "password"} placeholder={t.password} value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent text-white placeholder:text-white/50 focus:outline-none" />
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0" )}><GlassButton type="button" onClick={handleProgressStep} size="icon" aria-label="Submit password" contentClassName="text-white/90 hover:text-white"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                        </div></div>
                                    </div>
                                    <BlurFade inView delay={0.2}>
                                <button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-sm group transition-all hover:scale-105 active:scale-95">
                                    <ArrowLeft className="w-4 h-4 text-blue-400 group-hover:-translate-x-0.5 transition-transform" />
                                    <span className="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent font-bold tracking-tight">{t.goBack}</span>
                                </button>
                            </BlurFade>
                                </BlurFade>}
                            </AnimatePresence>
                    ...
                    <AnimatePresence>
                        {authStep === 'confirmPassword' && <BlurFade key="confirm-password-field" className="w-full">
                            <div className="relative w-full">
                                <AnimatePresence>
                                    {confirmPassword.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-white/60 font-semibold">{t.confirmPassword}</label></motion.div>}
                                </AnimatePresence>
                                <div className="glass-input-wrap w-[300px]"><div className="glass-input">
                                    <span className="glass-input-text-area"></span>
                                    <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                        {isConfirmPasswordValid ? <button type="button" aria-label="Toggle confirm password visibility" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="text-white/90 hover:text-white transition-colors p-2 rounded-full">{showConfirmPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button> : <Lock className="h-5 w-5 text-white/90 flex-shrink-0" />}
                                    </div>
                                    <input ref={confirmPasswordInputRef} type={showConfirmPassword ? "text" : "password"} placeholder={t.confirmPassword} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="relative z-10 h-full w-0 flex-grow bg-transparent text-white placeholder:text-white/50 focus:outline-none" />
                                    <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isConfirmPasswordValid ? "w-10 pr-1" : "w-0" )}><GlassButton type="submit" size="icon" aria-label="Finish sign-up" contentClassName="text-white/90 hover:text-white"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                </div></div>
                            </div>
                            <BlurFade inView delay={0.2}>
                                <button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-sm group transition-all hover:scale-105 active:scale-95">
