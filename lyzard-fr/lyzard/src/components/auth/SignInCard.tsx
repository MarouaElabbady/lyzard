...
import { Mail, Lock, Eye, EyeOff, ArrowRight, ArrowLeft, X } from 'lucide-react';
...
export function SignInCard({ isModal, onClose }: SignInCardProps) {
  ...
  return (
    <motion.div 
      ...
      className={cn(
        ...
        isModal ? "bg-black/60 backdrop-blur-sm" : "bg-black w-screen min-h-screen"
      )}
    >
      {isModal && <div className="absolute inset-0" onClick={onClose} />}
      ...
      {!isModal && (
        <div className="fixed top-4 left-4 z-20 md:left-6 md:top-6">
            <Link to="/" className="flex items-center gap-2 group transition-all">
                ...
                <span className="text-sm font-bold tracking-tight bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent group-hover:opacity-80 transition-all">{t.backToHome}</span>
            </Link>
        </div>
      )}

      {isModal && (
        <button onClick={onClose} className="fixed top-6 right-6 z-[110] p-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white/70 hover:text-white transition-all hover:scale-110 active:scale-90 shadow-lg">
          <X className="w-6 h-6" />
        </button>
      )}
            ...
            />
              ...
              <div className="relative bg-black/40 backdrop-blur-xl rounded-2xl p-6 border border-white/[0.05] shadow-2xl overflow-hidden">
                ...
                <div className="text-center space-y-1 mb-5">
                  ...
                  >
                    <LyzardLogo size={120} variant="dark" className="drop-shadow-[0_0_20px_rgba(168,85,247,0.6)]" />
                  </motion.div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <div className="text-xs relative group/link">
                      <Link to="/forgot-password" title="Forgot Password" className="text-white/60 hover:text-white transition-colors duration-200">
                        {t.forgotPassword}
                      </Link>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="relative overflow-hidden bg-white/5 text-white font-medium h-10 rounded-lg border border-white/10 hover:border-white/20 transition-all duration-300 flex items-center justify-center gap-2">
                      ...
                      <span className="text-white/80 group-hover/google:text-white transition-colors text-xs">{t.continueWith} Google</span>
                ...
                <motion.p className="text-center text-xs text-white/60 mt-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
                  ...
                  >
                    <span className="relative z-10 bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent group-hover:opacity-80 transition-all font-bold">
                      {t.signUp}
                    </span>
                    <span className="absolute bottom-0 left-0 w-0 h-[1px] bg-gradient-to-r from-blue-400 to-pink-400 group-hover:w-full transition-all duration-300" />
                  </Link>
                </motion.p>
              </form>
            </div>
